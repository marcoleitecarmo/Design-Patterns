...
Vantagens:
    Permetirem criar um sistema com baixo acoplamento enttre classes porque
    ocultam as classes que criam os objetos do código cliente.

    Falicitam a adição de novas classes ao código, porque o cliente não
    conhece e nem utiliza a implementação da classe (utiliza a factory).

    Podem falicitar o processo de "chache" ou criação de "singletons" porque a
    fábrica pode retornar um objeto ja criado para o cliente, ao invés de criar
...
Simple Factory pode quebrar o princípios do SOLID
"""
